...
import Game.Zelda.Entities.Statics.MMDirectionTile;
...
public class MapBuilder {
	...
	public static Map createMap(BufferedImage mapImage, Handler handler,String name){
		...
		for (int i = 0; i < mapImage.getWidth(); i++) {
			for (int j = 0; j < mapImage.getHeight(); j++) {
				...
				}else if(currentPixel == DirectionUp) {
					MMDirectionTile ghost = new MMDirectionTile(xPos, yPos,Direction.UP, Images.directionTiles.get(0), handler);
					mapInCreation.addBlock(ghost);
				}else if(currentPixel == DirectionRight) {
					MMDirectionTile ghost = new MMDirectionTile(xPos, yPos,Direction.RIGHT, Images.directionTiles.get(1), handler);
					mapInCreation.addBlock(ghost);
				}else if(currentPixel == DirectionDown) {
					MMDirectionTile ghost = new MMDirectionTile(xPos, yPos,Direction.DOWN, Images.directionTiles.get(2), handler);
					mapInCreation.addBlock(ghost);
				}else if(currentPixel == DirectionLeft) {
					MMDirectionTile ghost = new MMDirectionTile(xPos, yPos, Direction.LEFT, Images.directionTiles.get(3), handler);
					mapInCreation.addBlock(ghost);
	...
	public static BufferedImage arrayToRGBImage(ArrayList<ArrayList<BufferedImage>> info,String name,ArrayList<ArrayList<int[]>> teleportList){
		...
		for (int y = 0; y < info.get(0).size(); y++) {
			for (int x = 0; x < info.size(); x++) {
				...
				}else if(Images.directionTiles.get(0).equals(info.get(x).get(y))) {
					image.setRGB(x,y,DirectionUp);
				}else if(Images.directionTiles.get(1).equals(info.get(x).get(y))) {
					image.setRGB(x,y,DirectionRight);
				}else if(Images.directionTiles.get(2).equals(info.get(x).get(y))) {
					image.setRGB(x,y,DirectionDown);
				}else if(Images.directionTiles.get(3).equals(info.get(x).get(y))) {
					image.setRGB(x,y,DirectionLeft);
				}
	...
	public static int DirectionUp= new Color(10, 10, 0).getRGB();
	public static int DirectionRight= new Color(10, 10, 1).getRGB();
	public static int DirectionDown= new Color(10, 10, 2).getRGB();
	public static int DirectionLeft= new Color(10, 10, 3).getRGB();
