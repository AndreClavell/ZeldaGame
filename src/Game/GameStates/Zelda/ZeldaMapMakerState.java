...
public class ZeldaMapMakerState extends State {
	...
	public void tick() {
		...
		//Moves the selected tile to the middle of the tile set
		if (handler.getKeyManager().keyJustPressed(KeyEvent.VK_B)){	
			selectedList.set(selectedList.size()/2,selectedList.get(counter));
		}

		//Chooses a random tile from the current tile set
		if (handler.getKeyManager().keyJustPressed(KeyEvent.VK_R) && !handler.getKeyManager().shift){	
			counter = rand.nextInt(selectedList.size());
		}
		//Chooses a random tile from a random tile set
		if (handler.getKeyManager().keyJustPressed(KeyEvent.VK_R) && handler.getKeyManager().shift){	
		...
		if (handler.getKeyManager().keyJustPressed(KeyEvent.VK_T)){
			if (linking){
				...
				case 4:
					...
					break;
				case 5:
					selectedList = Images.directionTiles;
					break;
				}
			}
		}
